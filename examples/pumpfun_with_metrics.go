...
//
// Run: GEYSER_API_TOKEN=your_token go run examples/pumpfun_with_metrics.go
...
func main() {
	...
	fmt.Println("Starting Sol Parser SDK Example with Metrics...")
	fmt.Println("🚀 Subscribing to Yellowstone gRPC events...")
		...
		OnUpdate: func(update *solparser.SubscribeUpdate) {
			...
			for _, ev := range events {
				for key := range ev {
					...
					fmt.Printf("================================================\n")
					fmt.Printf("Event: %s\n", key)
					if v, ok := dataMap["mint"]; ok {
						fmt.Printf("  mint  : %v\n", v)
					}
					if v, ok := dataMap["sol_amount"]; ok {
						fmt.Printf("  sol_amount: %v\n", v)
					}
					if v, ok := dataMap["token_amount"]; ok {
						fmt.Printf("  token_amount: %v\n", v)
					}
	...
	fmt.Printf("📋 Event Filter: Buy, Sell, BuyExactSolIn, Create\n")
